...
\item[Regression] Consists of output variables $\mathbf{y}$ and input variables $\mathbf{X}$. The number of example is $N = 1400$ and each input $\mathbb{x_n}$ has dimensionality $D = 48$. The first $34$ are real valued and the last $14$ are categorical.
\item[Classification]
...
Most variables seem normally distributed with various means, but the variables 43-48 seem categorical (42 not really normal, looks like the distribution of $y$ actually, 2 Gaussian with different means)
...
The matrix $\tilde{X}$ has rank 36 for regression. That means 13 variables are not correlated to anything (given a threshold of 10 percent of the maximum correlation). Then $\tilde{X}$ is not ill conditioned.
\section{Summary}
